...
# 搜索 SSR 风控敏感, 由共享状态和独立限速桶控制。
DETAIL_CONCURRENCY = 400         # 每 worker 详情并发协程数；连接池显式跟随该值
SEARCH_CONCURRENCY = 100         # 每 worker 搜索并发协程数；连接池显式跟随该值
DETAIL_RATE_PER_SEC = 400.0      # 详情桶目标速率 (所有 worker 合计)
SEARCH_RATE_PER_SEC = 100.0      # 搜索桶目标速率 (所有 worker 合计)
