...
> 目标：分析智联招聘服务端爬虫校验与反爬策略（心跳机制、用户行为打分、惩罚链），提高单个 IP 的采集稳定性。
> 日期：2026-08-10
...
| ③ 交互验证 | TCaptcha | IP 信誉差触发，绑定浏览器指纹 | 🔬 链路已确认，未集成 |
...
- legacy SSR 详情和登录态接口仍采用低频请求节奏（默认 `--sleep 1.5`），用于降低交互验证风险。
- 生产推荐的 `collect.py` Redis 路径不以逐请求 sleep 控制吞吐，而是使用按出口隔离的 Redis 滑动窗口桶：搜索目标 100 req/s，详情目标 400 req/s；每个 worker 内的搜索和详情分别使用 100、400 的独立并发信号量。
- 上述数值是当前配置目标，不构成智联服务端的长期安全上限。搜索并发 200-300 的 30 秒测试约 174 RPS 且无失败；详情 5 个有效岗位、连接池 500 的 10 秒测试为 573.8 RPS（2 次 211），3 个岗位、连接池 40 持续 180 秒为 207.6 RPS。长期稳态、验证码累积效应和多出口扩展尚未完成验证。
...
4. **行为节奏**：legacy SSR 和登录态接口保持低频；Redis 生产路径遵循 search/detail 分桶限速，并持续监控 p95、211、challenge 和验证码，不以固定 sleep 替代共享限速器
5. **EO Token 消耗**：重放受 IP 信誉限制，IP 被标记时重放会升级验证码；采集器检测到验证码后进入冷却（已实现 `--captcha-cooldown`）
...
- legacy 路径自适应速率：challenge ×2 / 冷却 ×4（配合 `http_client` 限速）；Redis 生产路径采用共享冷却状态和固定 search/detail 滑动窗口桶
- 指数冷却 60s→120s→240s…（上限 30min），跨 run 记忆，下次启动自动等完
- **EO-Bot-Js-Token 1h 缓存复用**（对应 §9.5：不再为每个请求重复执行 Node 挑战）
- 搜索路径遇验证码自动冷却重试（3 轮），不再硬中断整批
- 对应 §9 建议：3（deviceSn 作为登录态接口的后续预留）、4（按路径区分低频 legacy 与 Redis 分桶限速）、5（遇验证码进入共享冷却并重试）

### 10.2 单 IP 规模化压测（路线一：搜索 SSR → position-detailv2）

历史基线来自 `js_reverse_cache/stress_test.py`，匿名，5 关键词 × 5 页搜索池 → 448 条详情连续采集。当前并发边界来自 `tools/search_rps_probe.py` 与 `tools/detail_rps_probe.py`：
...
| 接口 | 测试条件 | 结果 | 解释 |
|---|---|---|---|
| 搜索 SSR | 并发 200/300，持续 30 秒 | 173.5/174.6 RPS，0 失败 | 有效吞吐平台约在 200-300 并发 |
| 搜索 SSR | 并发 400/500，持续 10 秒 | 150.7/161.5 RPS，p95 约 4 秒 | 继续提高并发导致尾延迟恶化 |
| `position-detailv2` | 5 个岗位，连接池 500，持续 10 秒 | 573.8 RPS，2 次 211，0 其他异常 | 短时峰值，不代表长期稳态 |
| `position-detailv2` | 3 个岗位，连接池 40，持续 180 秒 | 207.6 RPS，37,443 成功、78 次 211 | 当前最有参考价值的稳态样本 |

连接池 600/800 的详情测试触发 Windows asyncio selector 的 `select()` 文件描述符限制，属于本地运行时约束；该结果不能用于推断服务端上限。

**结论**：路线一（position-detailv2）在当前测试条件下可通过高并发纯协议采集，且不依赖登录态；生产默认仍以 400 req/s 作为详情桶目标，需通过 10-30 分钟生产形态压测后再决定是否继续提高。
搜索 `positionCount` 恒显示 100 但实际可翻页远超 5 页（实测 p1-p8 全唯一、p=50 仍有数据）。

### 10.3 边界提醒

- 路线二（SSR 挑战）在规模化运行时仍会消耗 IP 信誉，仅作为低频兜底路径使用。
