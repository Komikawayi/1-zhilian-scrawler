...
```
搜索队列  zhaopin:tasks:search   (LIST, 与详情同速率, N worker)
  keyword:城市:关键词:页码        ← 阶段一 关键词搜索任务 (producer 入队)
...
- 压测证据（2026-08-08）：sou 搜索 0.1s 间隔 / 15/s 令牌桶 / 4 路并发 7.7req/s **全部 0 触发防线**，搜索对高频不敏感，与详情同档
...
**限速（共享单桶）**：`RedisRateLimiter` 滑动窗口 Lua，**搜索+详情共用一个令牌桶**（15/s）——因为两者消耗同一 IP 信誉资源，分开配速等于把总速率翻倍超过验证阈值。并发由各队列 worker 数控制。
...
- **搜索与详情同速率**（实测 2026-08-08）：sou 搜索 0.1s 间隔 / 15/s 令牌桶 / 4 路并发 7.7req/s 全部 0 触发防线，与详情接口表现一致
- **共享单令牌桶**：搜索+详情合计 15/s（同一 IP 信誉资源，分开配速=总速率翻倍超验证阈值）；并发由各队列 worker 数独立控制
...
配置播种：
- `config/keywords.json` ← 从 51job-crawler 播种 112 个制造词（`config/51job_city_profiles.json` 去重，SMT/PCBA/贴片/AOI/SPI/锡膏/EMS/NPI/控制板…）
