...
utils/async_client.py       异步客户端 (curl_cffi AsyncSession, curl_infos 网络分段) + Redis 全局匀速限速/HTTP 计数
...
- **全链路稳态（2026-08-10）**：搜索以新关键词“康复治疗师（北京）”在 20 秒稳态窗口完成 **2,009 次有效 SSR 响应（100.45/s）**。详情以 25 万真实任务池、2 个 worker、每 worker 400 协程、全局 400/s 桶运行约两分钟：`45,707` 已发出、`45,632` 已响应、`45,633` 已持久化，完成速率约 **352/s**；无最终失败。`400/s` 是当前配置上限，不是此环境下的完成速率承诺。
...
```
[搜索] 排队   12 处理中   3 完成  412  失败  1 | 100.0/s   [详情] 排队   8 处理中  2 完成 1834 失败 0 | 352.0/s
...
- 行1：搜索/详情双桶排队/处理中/完成/失败 + 各自实时完成速率（**完成/失败为本次运行增量**，Redis done/failed 历史累积已由启动基线排除）。一秒采样会受批量提交和调度影响波动；需要核验 HTTP 吞吐时读取对应桶的 `issued`/`responded` 计数。
...
- **本地埋点**（解析 / DB 写 / 风控预检 / 冷却等待）：详情入库包含进入有界批量写入器后的等待和提交时间；它用于识别端到端背压，不与单条 SQL 耗时混用。Redis 桶额外记录 `issued`（已发出）和 `responded`（已收到 HTTP 响应），用于核验真实 HTTP 吞吐。
...
- legacy SSR 或登录态接口仍采用低频节奏（默认 `--sleep 1.5`）；生产 Redis 路径使用独立并发和 Redis 匀速时隙限速，不应将两种路径的请求节奏混为一谈。
